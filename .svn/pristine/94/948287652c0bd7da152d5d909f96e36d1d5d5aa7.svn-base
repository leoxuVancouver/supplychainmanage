...
var Calendar = function( instanceId, language, startYear, endYear ){
	if( typeof instanceId == "string" ){
		...
		this.notesDayClass = 		'calendarNotesDay';
...
Calendar.prototype._bindAllData = function( curYear, curMonth ){
	...
	for( var i = 0; i < spans.length; i ++ ){
		...
		if($("#linkQuickMonth").html() == "11" && daysOfMonthArray[i] == "12")
		{
				spans[i].className = this.notesDayClass;
		}
		if($("#linkQuickMonth").html() == "10" && daysOfMonthArray[i] == "10")
		{
				spans[i].className = this.notesDayClass;
		}
	}
